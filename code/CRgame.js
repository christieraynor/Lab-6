...
Level.prototype.obstacleAt = function (pos, size) {
    var xStart = Math.floor(pos.x); //Removes everything after a decimal; Rounds down
    var xEnd = Math.ceil(pos.x + size.x); //Rounds up
    var yStart = Math.floor(pos.y);
    var yEnd = Math.ceil(pos.y + size.y);

    // Consider the sides, top and bottom of level as walls
    if (xStart < 0 || xEnd > this.width || yStart < 0 || yEnd > this.height)
      return "wall";
    // Check each gride position starting at yStart and xStart
    // Check for possible obstacles (non null value)
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  //Find if there is a new obstacle at the newPos
